...
const requiredFields = ["name", "email", "phone", "eventType", "eventDate", "guestCount"] as const;
...
export async function POST(request: Request) {
  ...
  for (const field of requiredFields) {
    if (!cleanLine((data as Record<string, unknown>)[field])) {
  ...
  const eventType = cleanLine(data.eventType);
  const eventDate = cleanLine(data.eventDate);
  const guestCount = cleanLine(data.guestCount);
  const budgetRange = cleanLine(data.budgetRange);
  const venue = cleanLine(data.venue);
  const cateringOption = cleanLine(data.cateringOption);
  ...
  if (eventType === "Other" && !message) {
    return NextResponse.json({ error: "Please tell us about your event." }, { status: 400 });
  }

  const combinedMessage = [message, cateringOption ? `Preferred catering option: ${cateringOption}` : null]
    .filter(Boolean)
    .join("\n\n");
